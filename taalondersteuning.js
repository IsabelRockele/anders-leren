...
window.Taalgroei = (function () {
  ...
  function huidigSchooljaar() {
    if (typeof lkActiefSchooljaarId === 'function') {
      const actief = lkActiefSchooljaarId();
  ...
  function geefTaak(id){const g=groepen.find(x=>x.id===id);if(!g)return;const codes=Array.isArray(g.deelnemerCodes)?g.deelnemerCodes:[];if(!codes.length)return alert('Kies eerst leerlingen voor deze groep.');if(typeof lkBeheerTaakVoorGroep!=='function')return alert('De taakfunctie is nog niet geladen. Vernieuw de pagina.');lkBeheerTaakVoorGroep(codes,g.naam);}
  ...
  return {open,render,slaGroepOp,wisForm,bewerk,openDeelnemers,bewaarDeelnemers,geefTaak,openEvaluaties,renderEvaluatieForm,bewaarEvaluatie,sluitDetail};
})();
